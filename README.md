...
| 18 | [Dashboard ตั้งเบิกทำจ่าย จากไฟล์ Costing (SAP export)](docs/18-costing-payment-dashboard.md) | **สกัดข้อมูลจากไฟล์ Costing ที่ออกจาก SAP ขึ้น Dashboard ตั้งเบิกทำจ่าย** — รับได้ทั้งฝั่ง **Mech และ Food** ไฟล์คนละใบ กฎเดียวกัน แยกสายจากเลข PO ให้เอง — ตัดรายการ LC ออก · เงื่อนไขจ่ายว่างแปลงเป็น UNKNOWN ไม่ใช่ทิ้งแถว · ตรวจความครบ 4 ฟิลด์แล้วบอกว่าขาดฟิลด์ไหน · ติดธงรายการยอดศูนย์กันตั้งเบิกผิด · ผู้ใช้เอาไฟล์ใหม่ทับแล้วกดปุ่มเดียว |
| 19 | [ต่อ Dashboard ตั้งเบิกทำจ่าย เข้ากับ Procure-to-Pay](docs/19-costing-to-p2p-link.md) | **วิธีต่อสองระบบ** — Dashboard เป็นต้นทางของงวดจ่าย ไม่ใช่ที่อนุมัติ · **กันตั้งเบิกซ้ำด้วยลายนิ้วมือแถว** เมื่อเอาไฟล์ใหม่ทับ (ชีต `Ledger` ห้ามลบ) · ลิงก์ตรงถึงรายการด้วย `#po/<เลข>` · ต่อได้ 3 ระดับ เลือกได้ว่าจะไปถึงระดับไหน |
...
| [`presentation/prototype.html`](presentation/prototype.html) | **ตัวอย่างระบบกดเล่นได้** — 5 เมนู · PO แยกกำลังดำเนินการ/เสร็จสิ้น · **สายอาหารกับสายเครื่องจักรใช้ช่องใบตรวจคนละชุด** · **ข้อค้างท้ายใบตรวจล็อกการจ่าย** · **เอกสารเรียกเก็บรายงวด (มัดจำ/งวดที่เหลือคนละใบ)** · **หน้าเชื่อมแอปตรวจรับ QC + คิวจับคู่ใบตรวจ** · **รายการซื้อเงินสด (ไม่มี PO) + ข้าม QC** · ค้นหา · **ฟอร์มบันทึกการจ่ายพร้อมสายอนุมัติ** · **ป๊อปอัปแนบไฟล์ + หมายเหตุ ทุกขั้นตอน** · **ฟอร์มขอทำจ่ายพร้อม Preview ข้อความที่จะประทับลงเอกสาร** · **ชุดเอกสารรวมเล่มอัตโนมัติ** · ใบคำร้อง (ขอราคา + เคลม + รหัสสินค้า) · ตรวจสอบก่อนบันทึก · กล่องแจ้งเตือน · ปุ่มแจ้งปัญหา · หน้าตั้งค่าระบบ |
| [`presentation/costing-dashboard.html`](presentation/costing-dashboard.html) | **Dashboard ตั้งเบิกทำจ่าย** จากไฟล์ Costing Mech — 97 แถวที่พร้อมเข้ากระบวนการ (ตัด LC ออก 134 แถว) ค้นหาเลข PO/ผู้ขาย กรองตามสถานะ สรุปรายใบ PO ดูรายการ LC ที่ตัดออก และ**ส่งเข้าระบบ P2P พร้อมกันส่งซ้ำ** |
...
node tools/smoke.js         # ชุดทดสอบ 287 การเรนเดอร์ (7 บทบาท) + เคสของทุกฟีเจอร์
python3 tools/build-schema-xlsx.py   # สร้างไฟล์ Excel โครงสร้างหลังบ้านใหม่ (ต้องมี openpyxl)
python3 tools/costing-extract.py "Costing Mech.xlsx" "Costing Food.xlsx"   # สกัดข้อมูลตั้งเบิกทำจ่าย (หลายสายได้)
python3 tools/costing-extract-test.py                  # ชุดทดสอบกฎการสกัดข้อมูลทุกข้อ
node tools/costing-parity.mjs grid.json py.json        # ตรวจว่า Python กับ Apps Script ให้ผลตรงกัน
```
