...
/* ================= 14. ขั้นเริ่มต้นของรายการที่นำเข้า =================
   ผิดมาแล้วรอบหนึ่ง: ตั้งไว้ที่ขั้นรับใบแจ้งหนี้ (12) ทำให้ไม่มีใครแนบ PO ที่เซ็นแล้วเลย
   แล้วตอนตรวจ 3 ทางก่อนปิดบัญชีจะไม่มี PO ให้เทียบ
   ทีมจัดซื้อเป็นคนทักว่า "ต้องเป็นหน้าที่ SR แนบ PO ก่อน" */
const startStage = 8;
ck(api.STAGES[startStage].o === 'SR',
   'ขั้นเริ่มต้นของรายการที่นำเข้าต้องเป็นงานของจัดซื้อ (ได้ ' + api.STAGES[startStage].o + ')');
const poDoc = api.DOCS.filter(d => d.c === 'PO_SIGNED')[0];
ck(poDoc && poDoc.at >= startStage,
