...
/* อ่านรายชื่อบทบาทและสิทธิ์เห็นเงินจากไฟล์จริง — ไม่เขียนรายชื่อตายตัวไว้ที่นี่
   เพิ่มบทบาทใหม่ (บัญชีต่างประเทศ/ในประเทศ · Sourcing Food/Mech · ผู้ดูแลระบบ)
   แล้วทั้งการไล่เรนเดอร์และเครื่องตรวจยอดเงินรั่วครอบคลุมให้เองทันที
   เคยพลาดมาแล้ว: เพิ่มบทบาทแต่ลืมแก้รายชื่อตรงนี้ เครื่องตรวจเลยไม่เคยวิ่งผ่านบทบาทใหม่ */
const MONEY_ROLE = {};
src.replace(/(\w+)\s*:\s*\{\s*name:[^{}]*?money:\s*(true|false)/g,
  (_, k, v) => { MONEY_ROLE[k] = v === "true"; return ""; });
const ROLES = Object.keys(MONEY_ROLE);
const MUST_HAVE = ["SR", "QC", "LS", "WH", "AC", "ACH", "GM"];
const missingRole = MUST_HAVE.filter(r => ROLES.indexOf(r) < 0);
if (missingRole.length)
  throw new Error("อ่านบทบาทจาก prototype.html ไม่ครบ ขาด: " + missingRole.join(" · "));
...
/* วางไว้ตรงนี้โดยตั้งใจ — ต้องตรวจก่อนโฟลว์ด้านล่างที่กดปุ่มเดินขั้นจนใบเปลี่ยนเจ้าของ
   เคยพลาดมาแล้ว: วางไว้ท้ายไฟล์แล้วคิวว่างเปล่า เพราะใบถูกส่งต่อไปแผนกอื่นหมดแล้ว */
/* ---------- ทีมย่อยต้องได้คิวของตัวเองจริง ----------
   จัดซื้อแยกตามสายสินค้า · บัญชีแยกตามสกุลเงิน
   ถ้าแบ่งแค่ในชื่อบทบาทแต่คิวยังรวมกันอยู่ ก็ไม่ได้แบ่งอะไรเลย
   ตรวจสองทางเสมอ: ต้องเห็นของตัวเอง และต้องไม่เห็นของอีกทีม
   (ทดสอบทางเดียวจะผ่านได้ด้วยหน้าจอที่โชว์ทุกใบให้ทุกคน) */
const SPLIT = [
  ["SR_FD", "PO-26-0045", "PO-26-0070", "Sourcing Food"],      // อาหาร ↔ เครื่องจักร ขั้นเดียวกัน (13)
  ["SR_MC", "PO-26-0070", "PO-26-0045", "Sourcing Mech"],
  /* สายค่าใช้จ่าย (PO-O…) ไม่ใช่ของทีมย่อยไหน — ตกเป็นของจัดซื้อทั่วไป
     ทีมย่อยจึงต้องไม่เห็นในคิวตัวเอง (ตรวจฝั่งจัดซื้อทั่วไปแยกด้านล่าง
     เพราะมันเป็นหัวหน้างานที่ครอบทีมย่อย จะเห็นของทีมย่อยด้วยโดยตั้งใจ) */
  ["SR_FD", "PO-26-0045", "PO-O326070012", "Sourcing Food (ค่าใช้จ่าย)"],
  ["SR_MC", "PO-26-0070", "PO-O326070012", "Sourcing Mech (ค่าใช้จ่าย)"],
  ["AC_FN", "PO-26-0033", "PO-26-0029", "บัญชีต่างประเทศ"],    // USD ↔ THB
  ["AC_TH", "PO-26-0029", "PO-26-0033", "บัญชีในประเทศ"]
];
/* หน้าแรกมีสามตาราง: งานของฉัน · กำลังจะถึงคิว · ของแผนกอื่น
   ต้องตัดมาเฉพาะตารางแรก ไม่งั้นสองตารางหลังมีใบของทุกทีมอยู่แล้ว เครื่องตรวจจะผ่านฟรี */
function myQueueHtml() {
  const h = nodes["#main"].innerHTML;
  // "ต้องทำวันนี้" โผล่สองที่ — ป้ายตัวเลขสรุปด้านบน กับหัวตารางงานของฉัน เอาอันหลัง
  const a = h.lastIndexOf("ต้องทำวันนี้");
  if (a < 0) return "";
  // หัวตารางถัดไปมีคำต่อท้าย จึงไม่ชนกับป้ายตัวเลขที่เขียนสั้นกว่า
  const b = h.indexOf("กำลังจะถึงคิวคุณ — เตรียมตัวได้", a);
  return h.slice(a, b < 0 ? h.length : b);
}
SPLIT.forEach(([r, own, other, label]) => {
  click("data-role", { role: r });
...
[["SR", ["PO-26-0045", "PO-26-0070", "PO-O326070012"], "จัดซื้อทั่วไป"],
 ["AC", ["PO-26-0033", "PO-26-0029"], "หัวหน้าบัญชี"]].forEach(([r, pos, label]) => {
  ...
  const mine = myQueueHtml();
  pos.forEach(po => {
    if (mine.indexOf(po) < 0) bad.push(label + " ดูแทนทีมย่อยไม่ได้ — ไม่เห็น " + po);
  });
});
...
// แผนกหน้างานที่ไม่มีหน้าที่ดูแลระบบ — เขียนเจตนาไว้ตรง ๆ ไม่อ่านจากไฟล์ที่กำลังตรวจ
// (ถ้าไปอ่านรายการสิทธิ์จาก prototype มาเทียบกับตัวเอง เครื่องตรวจจะผ่านเสมอ ไม่มีความหมาย)
const NO_ADMIN = ["QC", "LS", "WH"];
...
for (const r of NOMONEY) {
  ...
  /* หน้าตั้งค่าระบบมีทะเบียนซื้อเงินสดที่มียอดเงินอยู่ข้างใน
     แผนกหน้างานที่ไม่มีหน้าที่ดูแลระบบจึงไม่ควรเห็นเมนูนี้เลยตั้งแต่แรก

     ผู้ดูแลระบบ (IT) เป็นข้อยกเว้นที่ตั้งใจ — ต้องเข้าไปดูแลผู้ใช้ ทะเบียนเอกสาร และการเชื่อมต่อ
     แต่ยังห้ามเห็นยอดเงิน "สิทธิ์แก้ระบบ" กับ "สิทธิ์เห็นตัวเลข" เป็นคนละเรื่องกัน
     ข้อห้ามเรื่องเงินของ IT ไม่ได้หายไป — การสแกนด้านบนไล่ทุกหน้าของ IT รวมหน้านี้ด้วยแล้ว */
  if (NO_ADMIN.indexOf(r) >= 0 && /data-page="admin"/.test(nodes["#nav"].innerHTML))
    bad.push(r + " ยังเห็นเมนูตั้งค่าระบบบนแถบนำทาง");
  if (NO_ADMIN.indexOf(r) < 0 && !/data-page="admin"/.test(nodes["#nav"].innerHTML))
    bad.push(r + " เข้าหน้าตั้งค่าระบบไม่ได้ ทั้งที่มีหน้าที่ดูแลระบบ");
}
...
/* ---------- ใบหลายรายการในตัวอย่าง ----------
   ตัวอย่างต้องเล่าเรื่องเดียวกับระบบจริง ไม่ใช่แค่มีใบหลายรายการโผล่มาเฉย ๆ
   สามอย่างที่ต้องเป็นจริง: เห็นรายการครบ · ปิดไม่ได้จนกว่าจะตรวจ · ยอดรวมถูก */
click("data-role", { role: "AC_TH" });
click("data-po", { po: "PO-26-0075" });
const mlHtml = nodes["#main"].innerHTML;
["อินเวอร์เตอร์ SG125CX-P2", "ชุดยึดแผง", "สายไฟ DC"].forEach(it => {
  if (mlHtml.indexOf(it) < 0) bad.push("ใบหลายรายการ: ไม่แสดงรายการ " + it);
});
if (mlHtml.indexOf("฿592,000") < 0)
  bad.push("ใบหลายรายการ: ไม่แสดงยอดรวมทั้งใบ 592,000");
if (mlHtml.indexOf("ยืนยันว่าตรวจจากใบรับสินค้าแล้ว") < 0)
  bad.push("ใบหลายรายการ: ไม่มีปุ่มให้ยืนยันผลตรวจ — ปิดใบไม่ได้และไม่มีทางไปต่อ");
// ต้องยังไม่ผ่านด่าน ตราบใดที่ยังไม่มีใครตรวจ
if (mlHtml.indexOf("ระบบเทียบจำนวนให้ไม่ได้") < 0)
  bad.push("ใบหลายรายการ: ไม่ได้บอกว่าระบบเทียบจำนวนให้ไม่ได้ ผู้ใช้จะไม่รู้ว่าต้องทำอะไร");
// คลัง/QC ต้องเห็นรายการได้ แต่ต้องไม่เห็นยอดต่อรายการ (เครื่องสแกนเงินด้านล่างครอบอยู่แล้ว)
click("data-role", { role: "WH" });
click("data-po", { po: "PO-26-0075" });
